...
const HERO_HEADLINES = {
  ...
  d: {
    lines: [
      "הגיע הזמן להפסיק למכור",
      { text: "", accent: "את עצמכם בכל שיחת מכירה" },
    ],
  },
};
...
function Hero({ onCTAClick }) {
  ...
  // playback keeps its position. A genuine gesture is what lets setMuted(false)
  // actually take effect (browsers block programmatic unmute without one).
  React.useEffect(() => {
    ...
    loadSdk().then((Vimeo) => {
      if (Vimeo && heroIframeRef.current) player = new Vimeo.Player(heroIframeRef.current);
    });
    ...
    <section id="how" ref={sectionRef} style={{ padding: "96px 0", position: "relative", overflow: "hidden" }}>
      ...
      <style>{`
        ...
        .refund__headline {
          position: relative;
          z-index: 1;
          font-size: clamp(21px, 2.5cqw, 33px);
          font-weight: 900;
          line-height: 1.22;
          letter-spacing: -0.02em;
          text-wrap: balance;
      ...
      <div className="wrap" style={{ position: "relative", zIndex: 1 }}>
        ...
        <div className="cq-stack" style={{
          ...
          {[
          { n: "01", t: "מקליטים פרק היכרות", s: "באים לאולפן לפרק אחד, בלי להתחייב מראש לתהליך ארוך." },
          { n: "02", t: "מקבלים את התוצאה", s: "פרק מלא ערוך ומוכן לפרסום + 2 רילסים לרשתות." },
        ...
        <div style={{
          ...
          <div className="cq-stack" style={{
            ...
            {[
            { t: "פגישת אסטרטגיית תוכן אישית", s: "נשב יחד לפגישת אסטרטגיה שבה נבין איך נכון להפוך פודקאסט לנכס עבור העסק שלכם.", price: "1500", popular: false },
            { t: "מדריך דיגיטלי: איך מכינים פרק שמוכר", s: "כל מה שצריך לדעת לפני ההקלטה — שאלות, מבנה, וכיצד לגרום לאורח לדבר על מה שמעניין את הקהל שלכם.", price: "290", popular: true },
            { t: "מדריך דיגיטלי: פתיחת פודקאסט בכל הפלטפורמות", s: "Spotify, Apple, YouTube — מדריך מלא להעלאה והגדרה בכל הפלטפורמות בפעם הראשונה.", price: "220", popular: false },
            { t: "מדריך דיגיטלי: איך עובדים עם ManyChat", s: "הגדרה מלאה של אוטומציה שמביאה לידים מהפודקאסט ישירות לוואטסאפ — צעד אחר צעד.", price: "250", popular: false }].
            map((b, i) =>
            <div key={i}
          ...
          <div className="refund" data-in={inView ? "1" : "0"}>
            ...
            <div className="refund__lead">
              ובגלל שאנחנו כל כך בטוחים במה שאתם הולכים לקבל אצלנו
            </div>

            <div className="display refund__headline">
              <span className="refund__accent">אם לא תהיו מרוצים מהשירות והתוצרים שלנו</span>
    ...
    <section ref={stripRef} className="guest-section" style={{
      ...
      <style>{`
        ...
        @container (max-width: 768px) {
          ...
          /* Both rows are sized so several tiles are on screen at once — the
             point of the section is the number of faces, not any one of them. */
          .guest-marquee-row--tall > div { width: 130px !important; }
          .guest-marquee-row--wide > div { width: 280px !important; }
          /* Durations are set per width so both rows travel at ~150px/s — a
             narrower tile means a shorter set, which needs less time. */
          .guest-marquee-row--tall { animation-duration: 13s !important; }
        ...
        @container (max-width: 420px) {
          ...
          .guest-marquee-row--tall > div { width: 110px !important; }
          .guest-marquee-row--wide > div { width: 250px !important; }
          .guest-marquee-row--tall { animation-duration: 11s !important; }
  ...
  return (
    <div ref={ref} className="pullquote" style={{
      ...
      <p className="display" style={{
        ...
        <br />
        <AnimatedLine inView={inView} delay={1120}>
          <span className="gbl-uline" style={{
            color: "var(--accent)",
            fontWeight: 900,
            textShadow: "0 0 30px rgba(255,213,0,0.45)"
          }}>
            הכנו לכם הצעה שלא חוזרת
          </span>
        </AnimatedLine>
      </p>

      <style>{`
        .gbl-uline { position: relative; display: inline-block; }
        .gbl-uline::after {
          content: "";
          position: absolute;
